...
(define-struct initialstate (mybird press_key pipes score) #:transparent)
(define-struct game_bird (x y v_x v_y) #:transparent)
(define-struct pipe (x y) #:transparent)
;(define first_pipe (pipe scene-width (+ added-number (random random-threshold))))
;(define second_pipe (pipe (+ scene-width pipe-width pipe-gap)  (+ added-number (random random-threshold)))) 
;(define bird1 (game_bird 50 50 0 0))
;(define game_state (initialstate bird1 "space" (list second_pipe first_pipe) 0))
;game_state
...
(define (get-initial-state)
  (let
      ([bird_1 (game_bird bird-x bird-initial-y 0 0)]
       [pipe_1 (pipe scene-width (+ added-number (random random-threshold)))])
  (initialstate bird_1 " " (list pipe_1) 0)))
...
(define (next-state-bird bird gravity)
  (struct-copy game_bird bird [y (+ (get-bird-y bird) (game_bird-v_y bird))]
               [v_y (+ (get-bird-v-y bird) gravity)]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy game_bird bird [v_y (* -1 momentum)]))
...
(define (change current-state pressed-key)
  (if (equal? pressed-key (initialstate-press_key current-state))
      (let
          ([bird_change (next-state-bird-onspace (initialstate-mybird current-state) initial-momentum)])
           (struct-copy initialstate current-state [mybird bird_change]))
      current-state)) 
;TODO 9
...
(define (move-pipes pipes scroll-speed)
  (helper-move-pipes pipes (* -1 scroll-speed) null))
(define (helper-move-pipes pipes scroll-speed rez)
  (if (null? pipes) (reverse rez)
      (helper-move-pipes (cdr pipes) scroll-speed (cons (struct-copy pipe (car pipes) [x (+ (get-pipe-x (car pipes)) scroll-speed)]) rez))))
...
(define (clean-pipes pipes)
  (filter clean-pipes-helper pipes))

(define (clean-pipes-helper pipe)
  (if (< 0 (+ (get-pipe-x pipe) pipe-width)) #t #f)) 
...
(define (add-more-pipes pipes)
  (if (= (length pipes) no-pipes) pipes
      (let
          ([pipe_to_add (pipe (+ (pipe-x (car pipes)) pipe-width pipe-gap) (+ added-number (random random-threshold)))]) 
        (cons pipe_to_add pipes))))
;TODO 14
...
(define (check-ground-collision bird)
 (if (<= ground-y (+ bird-height (game_bird-y bird))) #t #f))
...
(define (invalid-state? state)
  (or (check-ground-collision (initialstate-mybird state))
       (check-pipe-collisions (initialstate-mybird state) (initialstate-pipes state))))
...
(define (check-pepe-coll bird pipe)
  (let ([p1 (make-posn (game_bird-x bird) (game_bird-y bird))]
        [p2 (make-posn (+ bird-width (game_bird-x bird)) (+ bird-height (game_bird-y bird)))]
        [p3 (make-posn (pipe-x pipe) (- (pipe-y pipe) pipe-height))]
        [p4 (make-posn (+ pipe-width (pipe-x pipe)) (pipe-y pipe))]
        [p5 (make-posn (pipe-x pipe) (+ pipe-self-gap (pipe-y pipe)))]
        [p6 (make-posn (+ pipe-width (pipe-x pipe)) scene-height)])
    (or (check-collision-rectangles p1 p2 p3 p4) (check-collision-rectangles p1 p2 p5 p6))))
  

(define (check-pipe-collisions bird pipes)
  (cond
    [(null? pipes) #f]
    [(check-pepe-coll bird (car pipes)) #t]
...
(define (next-state state)
  (let ([newbird (next-state-bird (initialstate-mybird state) initial-gravity)]
        [newpipes (next-state-pipes (initialstate-pipes state) initial-scroll-speed)]
        [newkey (initialstate-press_key state)]
        [newscore (+ (initialstate-score state) 0.1)])
    (initialstate newbird newkey newpipes newscore)))
(next-state (get-initial-state))
(length (get-pipes (next-state (get-initial-state))))
...
(define (draw-frame state)
  (let*
      ([pipes-image (place-image (place-pipes (reverse (initialstate-pipes state)) initial-scene)
                                 (quotient scene-width 2) (quotient scene-height 2) initial-scene)]
       [score-image (score-to-image (initialstate-score state))]
       [image-part-1 (place-image score-image text-x text-y pipes-image)]
       [image-ground (place-image ground-image (/ scene-width 2)
                                  (+ ground-y (/ ground-height 2))  image-part-1)]
       ;[image-ground image-part-1]
       [bird_to_put (initialstate-mybird state)]
       [image-complete (place-image bird-image (+ (game_bird-x bird_to_put) (quotient bird-width 2))
                                    (+ (game_bird-y bird_to_put) (quotient bird-height 2)) image-ground)])
    image-complete))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipe pipe)
	(place-image (rectangle (+ pipe-width 1) pipe-self-gap "solid" "white")
                        (/ pipe-width 2) (+ (pipe-y pipe) (/ pipe-self-gap 2))
                  (rectangle pipe-width scene-height "solid" "green")))

(define (place-pipes pipes scene)
  (if(null? pipes) scene
     (place-pipes (cdr pipes) (place-image (place-pipe (car pipes))
                                           (+ (pipe-x (car pipes)) (/ pipe-width 2))
